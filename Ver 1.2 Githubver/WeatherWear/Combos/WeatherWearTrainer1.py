...
def retrain(evidence, label, request):
    ...
    newmodel.load_weights(f"/home/Philbert/Userdata/{request.user.username}/{request.user.username}")
    newmodel.fit(evidence, label, epochs=5)
    newmodel.save_weights(f"/home/Philbert/Userdata/{request.user.username}/{request.user.username}")
